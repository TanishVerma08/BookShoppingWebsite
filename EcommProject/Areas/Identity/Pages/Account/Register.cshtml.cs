    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            Input = new InputModel()
            {
                ...
                RoleList = _roleManager.Roles.Where(r=>r.Name !=SD.Role_Individual).Select(rl=>rl.Name).Select(rl=> new SelectListItem()
                {
                    Text = rl,
                    Value = rl
                })
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        var userDetails = await _signInManager.UserManager.FindByNameAsync(Input.Email);
                        ...
                        if (Input.Role == null && Input.CompanyId == null)
                        {
                            await _twilioSender.SendVerificationCodeAsync(phoneNumber,"sms");
                            return RedirectToPage("Verify2faCode", new { PhoneNumber = phoneNumber, User = userDetails });
                        }
                        else
                        {
                            await _twilioSender.SendVerificationCodeAsync(phoneNumber,"sms");
